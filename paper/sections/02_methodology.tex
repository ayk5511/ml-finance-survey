...
\begin{itemize}[leftmargin=*]
    \item \textbf{Scopus}: broad coverage of finance and CS journals.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Financial terms:} ``stock prediction'', ``portfolio optimization'', ``risk management'', ``options pricing'', ``market microstructure'', ``quantitative finance''.
    \item \textbf{ML terms:} ``machine learning'', ``deep learning'', ``neural network'', ``random forest'', ``gradient boosting'', ``reinforcement learning'', ``natural language processing'', ``transformer'', ``LSTM''.
\end{itemize}

The query was constructed as \texttt{(financial term) AND (ML term)} for every pair, with results aggregated and deduplicated across the five databases.
...
\paragraph{Released subset of the 227-study sample.} The supplementary CSV
(\Cref{sec:cross_cutting}) carries structured records for 119 of the 227
included studies. That subset breaks down as 99 RDS-scored entries and 20
unscored seminal or baseline references retained for the comparison tables
in \Cref{sec:appendix_tables}. The remaining 108 studies are cited in-line
in \Cref{sec:return_prediction} through \Cref{sec:alternative_data}; their
extraction-field rows live in the screening logs in the GitHub repository
and will be merged into the released CSV in version-tagged updates. All
quantitative claims in the paper are computed against the full 227-study
sample unless we say otherwise, and where the released 119-row subset
differs materially we report both.
