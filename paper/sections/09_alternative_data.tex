...
Alternative data, in the sense of non-traditional sources such as news, social media, satellite imagery, web traffic, and corporate filings, is the fastest-growing application area for ML in finance. NLP methods are the primary tool for extracting signals from textual alternative data.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{BloombergGPT} \citep{Wu2023}: a 50B-parameter LLM trained on Bloomberg's proprietary corpus of financial documents. Beats general-purpose LLMs on financial NLP benchmarks while staying competitive on general-domain tasks.

    \item \textbf{General-purpose LLMs} (GPT-4, Claude): \citet{LopezLira2023} show that GPT-3.5/4 can predict stock returns from news headlines, with performance comparable to purpose-trained models. The zero-shot capability eliminates the need for labeled training data.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{StockTwits:} a dedicated financial social platform whose posts ship with bullish or bearish labels, which makes it a convenient training set for sentiment models \citep{Oliveira2017}.
\end{itemize}
